    ...
    fn lower_id(&mut self, id: TypeId, dest: &AbiLoc<'_>) {
        ...
        match &self.resolve.types[id].kind {
            ...
            TypeDefKind::List(t) => {
                ...
                self.lower_list(list_index, &[*t], &l_ptr, &l_len, pop_iter_next, pop_iter);
                self.ins().end();
    ...
    fn lift_id(&mut self, src: &AbiLoc<'_>, id: TypeId) {
        ...
        match &self.resolve.types[id].kind {
            ...
            TypeDefKind::List(t) => {
                ...
                self.ins().i32_eqz();
                self.ins().if_(BlockType::Empty);
                self.lift_list(list_index, &[*t], &l_ptr, &l_len, list_append);
                self.ins().end();
            ...
            TypeDefKind::Map(k, v) => {
                ...
                self.lift_list(map_index, &[*k, *v], &l_ptr, &l_len, map_append);
    ...
    fn lift_list(
        ...
        self.ins().loop_(BlockType::Empty);
        {
            ...
            self.ins().if_(BlockType::Empty);
            {
                ...
                // Push the lifted elements onto the list.
                self.local_get_ctx();
                self.ins().i32_const(list_index.try_into().unwrap());
